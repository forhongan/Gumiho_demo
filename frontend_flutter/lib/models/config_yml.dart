class GumihoConfig {
  ...
  factory GumihoConfig.fromYaml(Map<String, dynamic> yaml) {
    ...
    return GumihoConfig(
      ...
      name: yaml['book/game/video Name'] ?? yaml['Name'] ?? '', // 添加备选字段
      bookContentSummary: yaml['book content summary'],
      writingStyle: yaml['Writing style'],
      enableBaseInformation: yaml['Enable base imformation'] ?? false, // 注意这里后端的拼写错误
  ...
  GumihoConfig copyWith({
    ...
    return GumihoConfig(
      translationProjectName: translationProjectName ?? this.translationProjectName,
      translater: translater ?? this.translater,
      type: type ?? this.type,
      originalLanguage: originalLanguage ?? this.originalLanguage,
      targetLanguage: targetLanguage ?? this.targetLanguage,
      paragraphed: paragraphed ?? this.paragraphed,
      originalFormat: originalFormat ?? this.originalFormat,
      defaultAISetting: defaultAISetting ?? this.defaultAISetting,
      firstTranslationNeeded: firstTranslationNeeded ?? this.firstTranslationNeeded,
      proofreadingNeeded: proofreadingNeeded ?? this.proofreadingNeeded,
      name: name ?? this.name,
      bookContentSummary: bookContentSummary ?? this.bookContentSummary,
      writingStyle: writingStyle ?? this.writingStyle,
      enableBaseInformation: enableBaseInformation ?? this.enableBaseInformation,
      firstTranslationSetting: firstTranslationSetting ?? this.firstTranslationSetting,
    );
  }
}
...
class AISetting {
  ...
  AISetting copyWith({
    ...
    return AISetting(
      ...
      jsonOrNot: jsonOrNot ?? this.jsonOrNot,
...
class FirstTranslationSetting {
  ...
  factory FirstTranslationSetting.fromMap(Map<String, dynamic> map) {
    print("DEBUG: 从Map加载FirstTranslationSetting: ${map.keys}");
    return FirstTranslationSetting(
      ...
      enableUniformLength: map['enable_uniform_length'] ?? map['enable uniform length'] ?? false, // 添加备选键
      enableMaximumLength: map['enable_maximum_length'] ?? map['enable Maximum length'] ?? false, // 添加备选键
      enableContents: map['enable contents'] ?? map['Enable Contents'] ?? false, // 添加备选键
  ...
  Map<String, dynamic> toMap() {
    final map = {
      ...
      'enable_uniform_length': enableUniformLength, // 修改为下划线风格，与后端一致
      'enable_maximum_length': enableMaximumLength, // 修改为下划线风格，与后端一致
    ...
    if (aiConfig != null) {
      map['ai_config'] = aiConfig!.toMap();
    }
    
    return map;
  }

  // 修改 copyWith 方法，添加新的参数

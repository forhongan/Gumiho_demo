...
class _PNTScreenState extends State<PNTScreen> {
  ...
  Future<void> _loadPNT() async {
    final uri = Uri.parse('http://127.0.0.1:5000/pnt?PNTPath=${widget.project.PNTPath}');
    try {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('PNT已保存')),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Text('编辑PNT - ${widget.project.name}', style: const TextStyle(color: Colors.white, fontSize: 20)),
                      const Spacer(),
                      IconButton(
                ...
                Expanded(
                  child: Padding(
                    ...
                    child: _isStructuredView
                        ...
                        : Column(
                            children: [
                              Text('PNT文件路径: ${widget.project.PNTPath}'),

        ...
        try:
            ...
            if stream:
                ...
                for chunk in response:
                    ...
                    self.reasoning_content += current_reasoning
                    # 处理 content
                    ...
                    full_content = current_reasoning + current_content
                    # 发送实时内容到前端
                    if self.sse_callback and full_content:
                        self.sse_callback({
                            "type": "progress",
                ...
                # 非流式响应处理
                self.reasoning_content = getattr(response.choices[0].message, 'reasoning_content', '')
                self.content = getattr(response.choices[0].message, 'content', '')
